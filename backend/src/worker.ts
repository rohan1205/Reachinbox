...
if (!redisConnection) {
  console.log("⚠️ Worker disabled - Redis not available");
} else {
  ...
  const worker = new Worker(emailQueueName, async (job: Job) => {
    ...
    const sentCount = await conn.incr(limitKey);
    
    if (sentCount === 1) {
      // TTL of 2 hours
      await conn.expire(limitKey, 60 * 60 * 2);
    }

    if (sentCount > hourlyLimit) {
      // Limit reached, rollback the increment for this hour
      await conn.decr(limitKey);
      ...
      const overflowSeq = await conn.incr(overflowSeqKey);
      if (overflowSeq === 1) {
        await conn.expire(overflowSeqKey, 60 * 60 * 3);
      }
